...
import org.obsrj.core.listeners.BlocksDownloadedEventListener;
import org.obsrj.core.listeners.GetDataEventListener;
...
import org.obsrj.crypto.TransactionSignature;
import org.obsrj.wallet.*;
...
public class MatiTest {
    ...
    public void accountDetailsTest() {
        ...
        Wallet wallet = Wallet.fromSeed(
                networkParameters,
                seed,
                DeterministicKeyChain.KeyChainType.BIP44_COIN_ONLY
        );
    ...
    public Wallet restore(NetworkParameters networkParameters) throws IOException {
        String filename = "2.0.32_wallet-backup_org.obsr.production-2017-11-20";
    ...
    public void watchOnlyTest() {
        ...
        Wallet watchingWallet = Wallet.fromWatchingKeyB58(networkParameters, xpub, 0, DeterministicKeyChain.KeyChainType.BIP44_COIN_ONLY);
    ...
    public void syncWatchingKey() throws IOException, BlockStoreException {
        ...
        //Wallet watchingWallet = Wallet.fromWatchingKeyB58(params,pubKey,0, DeterministicKeyChain.KeyChainType.BIP44_COIN_ONLY);
        //DeterministicKey xpubKey2 = watchingWallet.getWatchingKey();
        ...
        //dir.mkdir();
        File walletFile = new File("wallet_testn322233e3t_3.dat");
        ...
        peerGroup.addPeerDiscovery(new PeerDiscovery() {
            ...
            public InetSocketAddress[] getPeers(long services, long timeoutValue, TimeUnit timeoutUnit) throws PeerDiscoveryException {
                return new InetSocketAddress[]{
                        ...
                        new InetSocketAddress("35.227.74.136", 55004)
    ...
    public void signMultiSigTransaction() {
        ...
        // Add outputs to the person receiving
    ...
    public void connect() throws BlockStoreException, IOException, InsufficientMoneyException {
        ...
        //File dir = new File("dir");
        //dir.mkdir();
        File walletFile = new File("wallet_2tefffs2ewweffe3391t12ee2.dat");
        Wallet wallet = createWallet(networkParameters, walletFile);
//        String mnemonic = "host, clap, cross, spy, huge, until, accuse, blanket, suit, render, either, void, depend, enemy, math, palace, unware, setup, puppy, sauce, hair, check, isolate, near";
//        mnemonic = mnemonic.trim().replace(",", "");
//        List<String> list = new ArrayList<String>();
//        list.addAll(Arrays.asList(mnemonic.split(" ")));
//        DeterministicSeed seed = new DeterministicSeed(list, null, "", 0);
...
//        wallet.addWatchedAddress(Address.fromBase58(networkParameters, "TCH9ZBHG95q8DovbZhVW9N9SnPjJnJLx2M"));
//        wallet.addCoinsReceivedEventListener(new WalletCoinsReceivedEventListener() {
//            @Override
//            public void onCoinsReceived(Wallet wallet, Transaction tx, Coin prevBalance, Coin newBalance) {
//                logger.info("onCoinsReceived"+ tx.toString());
//            }
//        });
        //Wallet wallet = restore(networkParameters);
...
//        wallet.addWatchedAddress(Address.fromBase58(networkParameters,"DFuEgkFtkXrMQfFoZXckm88LuqzAAukGK3"));
        ...
        BlockStore spvBlockStore = new LevelDBBlockStore(context, new File("block_egawwwwffa336eeweww2fes2t12.dat")); //new LevelDBBlockStore(context,new File("blockstore.dat"));
        //InputStream inputStream = new FileInputStream(new File("checkpoints"));
        //CheckpointManager.checkpoint(networkParameters, inputStream, spvBlockStore, System.currentTimeMillis());
        final BlockChain blockChain = new BlockChain(context, wallet, spvBlockStore);
        ...
        peerGroup.addPeerDiscovery(new PeerDiscovery() {
            ...
            public InetSocketAddress[] getPeers(long services, long timeoutValue, TimeUnit timeoutUnit) throws PeerDiscoveryException {
                ...
                peers.add(new InetSocketAddress("localhost", 9567));
...
//        peerGroup.addPeerDiscovery(new PeerDiscovery() {
//
//            private final PeerDiscovery normalPeerDiscovery = MultiplexingDiscovery.forServices(networkParameters, 0);
//
//            @Override
...
//        });
        ...
        peerGroup.setMaxPeersToDiscoverCount(1);
        peerGroup.setConnectTimeoutMillis(10 * 1000);
        peerGroup.setPeerDiscoveryTimeoutMillis(15 * 1000);
        peerGroup.startBlockChainDownload(new AbstractPeerDataEventListener() {
            @Override
            public void onBlocksDownloaded(Peer peer, Block block, @Nullable FilteredBlock filteredBlock, int blocksLeft) {
...
//                            logger.info("hash: " + block.getHash().toString() + " op_return:" + new String(transactionOutput.getScriptPubKey().getPubKey()));
//                        }
//                    }
//                }
////                if (block.getHash().toString().equals("ce8a49f75cce9966f259b76c9470329eeb256db4b6c618daaaf4b56b48599b26") ||
////                        block.getHash().toString().equals("fb530e5ae850f2773ba7333e59d6ad7910da7e25d72692dd3652cd5d8b9d6742") ||
////                        block.getHash().toString().equals("5f1a74571d0aa31b3fedd26e1ba5bd50435d8a025d12d74a13271a055ac36ec2")
////                        ) {
//                    for (Transaction transaction : block.getTransactions()) {
//                        for (TransactionOutput transactionOutput : transaction.getOutputs()) {
//                            logger.info("block hash: " + block.getHash().toString() + "op_return txoutput:" + new String(transactionOutput.getScriptPubKey().getPubKey()));
//                        }
//                    }
////                }
                if ((blocksLeft / 10000) == 0) {
...
//            if (transaction.getUpdateTime().getTime() < 1528539480000L) {
//                continue;
//            }
//            for (TransactionOutput transactionOutput : transaction.getOutputs()) {
//                if (transactionOutput.getScriptPubKey().isOpReturn()) {
//                    logger.info("hash" + transaction.getHashAsString() + " time " + transaction.getUpdateTime() +
//                            " transactionOutput:" + transactionOutput.getScriptPubKey().toString());
//
//                }
//            }
////
//////            try{
//////                logger.info(" time "+ transaction.getUpdateTime()+ " transaction:" + transaction.getHashAsString());
////////                logger.info(" time "+ transaction.getUpdateTime()+ " transactionRelated:" + wallet.isTransactionRelatedToWatchedAddress(transaction));
//////                logger.info(" time "+ transaction.getUpdateTime()+ " transaction:" + transaction.toString());
//////                logger.info(" time "+ transaction.getUpdateTime()+ " transaction:" + wallet.isTransactionRelatedToWatchedAddress(transaction));
//////
//////            }catch (Exception e) {
//////                e.printStackTrace();
//////            }
////
//////            logger.info(  wallet.getTransaction(Sha256Hash.wrap("c03cfab417d7548e1096533a8b97ecc426f5e4f27ec6d3470b8899c58c5ed182")).toString());
//////                for (TransactionOutput transactionOutput : transaction.getOutputs()) {
//////                    if (transactionOutput.getScriptPubKey().isOpReturn()) {
//////                        logger.info("hash: " + transaction.getHashAsString() +" time "+ transaction.getUpdateTime()+ " op_return:" + transactionOutput.getScriptPubKey().toString());
//////                    }
//////                }
////
//        }
////        Transaction transaction = wallet.getTransaction(Sha256Hash.wrap("1695a07a8e3e0d2f1565b202a4554c14003760cc8be04503e987dbaf826adc2b"));


//        logger.info(" isCoinBase:" + transaction.isCoinStake());

//        for (TransactionOutput transactionOutput : transaction.getOutputs()) {
//            if (transactionOutput.getScriptPubKey().isOpReturn()) {
//                logger.info(" op_return:" + transactionOutput.getScriptPubKey().toString());
//            } else {
//                logger.info(" address:" + transactionOutput.isDust());
//                logger.info(" address:" + transactionOutput.getScriptPubKey());
//                logger.info(" address:" + transactionOutput.getScriptPubKey().getToAddress(networkParameters));
//
//            }
//        }
        while (true) {
