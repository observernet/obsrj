...
import org.obsrj.core.Message;
import org.obsrj.core.listeners.NewBestBlockListener;
import org.obsrj.core.listeners.ReorganizeListener;
import org.obsrj.core.listeners.TransactionConfidenceEventListener;
import org.obsrj.core.listeners.TransactionReceivedInBlockListener;
...
import org.obsrj.signers.LocalTransactionSigner;
import org.obsrj.signers.MissingSigResolutionSigner;
import org.obsrj.signers.TransactionSigner;
import org.obsrj.utils.BaseTaggableObject;
import org.obsrj.utils.ListenerRegistration;
import org.obsrj.utils.Threading;



import org.obsrj.wallet.Protos.Wallet.*;
import org.obsrj.wallet.WalletTransaction.*;
import org.obsrj.wallet.listeners.KeyChainEventListener;
import org.obsrj.wallet.listeners.ScriptsChangeEventListener;
...
import org.obsrj.wallet.listeners.WalletEventListener;
import org.obsrj.wallet.listeners.WalletReorganizeEventListener;
import org.slf4j.*;
...
public class Wallet extends BaseTaggableObject
    ...
    private Wallet(Context context, KeyChainGroup keyChainGroup) {
        ...
        //this.version = this.keyChainGroup.getActiveKeyChain().getKeyChainType() == DeterministicKeyChain.KeyChainType.BIP44_COIN_ONLY? 1 : 0;
     ...
     * key, after loading a wallet produced by previous versions of obsrj. If the wallet is encrypted the key
        ...
        if (block != null) {
            ...
            if (bestChain) {
                ...
                for (Transaction txDependency : currentTxDependenciesSorted) {
                    if (txDependency.getConfidence().getConfidenceType().equals(TransactionConfidence.ConfidenceType.IN_CONFLICT)) {
                        if (isNotSpendingTxnsInConfidenceType(txDependency, TransactionConfidence.ConfidenceType.IN_CONFLICT)) {
    ...
    private void killTxns(Set<Transaction> txnsToKill, @Nullable Transaction overridingTx) {
        ...
        while (!work.isEmpty()) {
            ...
            for (TransactionInput deadInput : tx.getInputs()) {
                ...
                if (connected.getConfidence().getConfidenceType() != TransactionConfidence.ConfidenceType.DEAD && deadInput.getConnectedOutput().getSpentBy() != null && deadInput.getConnectedOutput().getSpentBy().equals(deadInput)) {
    ...
    public boolean maybeCommitTx(Transaction tx) throws VerificationException {
        ...
        try {
            ...
            if (!doubleSpendUnspentTxns.isEmpty() ||
                ...
                !isNotSpendingTxnsInConfidenceType(tx, TransactionConfidence.ConfidenceType.DEAD)) {
                ...
                !isNotSpendingTxnsInConfidenceType(tx, TransactionConfidence.ConfidenceType.IN_CONFLICT)) {
                ...
                if(tx instanceof TransactionLockRequest) //TODO:InstantX - may need to adjust the ones above too?
                    tx.getConfidence().setIXType(TransactionConfidence.IXType.IX_REQUEST);//setConfidenceType(ConfidenceType.INSTANTX_PENDING);
            ...
            //Dash Specific
            if(tx.getConfidence().isIX() && tx.getConfidence().getSource() == TransactionConfidence.Source.SELF) {
    ...
    public void setTransactionBroadcaster(@Nullable TransactionBroadcaster broadcaster) {
        ...
        for (Transaction tx : toBroadcast) {
            ...
            if(confidenceType == TransactionConfidence.ConfidenceType.UNKNOWN /*&& tx.getConfidence().getSource() == Source.SELF*/)
            {
            ...
            checkState(confidenceType == TransactionConfidence.ConfidenceType.PENDING || confidenceType == TransactionConfidence.ConfidenceType.IN_CONFLICT,
