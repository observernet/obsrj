...
import org.obsrj.core.listeners.BlockChainListener;
import org.obsrj.store.FlatDB;
import org.obsrj.utils.ContextPropagatingThreadFactory;
import org.obsrj.store.MasternodeDB;
 ...
 * obsrj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating obsrj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use obsrj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a obsrj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using obsrj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing obsrj via a thread that has not had any context set on it.");
    ...
    public void initCoin(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented  MN stuff
