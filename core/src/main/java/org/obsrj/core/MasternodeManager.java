...
//                Misbehaving(pfrom->GetId(), nDoS);
    ...
    public void checkAndRemove(boolean forceExpiredRemoval)
    {
        ...
        try {
            ...
            while (it3.hasNext()) {
                Map.Entry<Sha256Hash, MasternodeBroadcast> mb = it3.next();
                if (mb.getValue().lastPing.sigTime < Utils.currentTimeSeconds() - (MasterNodeSystem.MASTERNODE_REMOVAL_SECONDS * 2)) {
            ...
            while (it4.hasNext()) {
                ...
                if (mp.getValue().sigTime < Utils.currentTimeSeconds() - (MasterNodeSystem.MASTERNODE_REMOVAL_SECONDS * 2)) {
