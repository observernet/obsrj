...
import static org.obsrj.script.ScriptOpCodes.*;
...
public class Script {
     ...
     *
     * <p>obsrj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
    ...
    public boolean isZerocoinMint(){
        byte[] program = getProgram();
        return program.length >0 &&
                (program[0] & 0xff) == OP_ZEROCOINMINT;
    }

    public boolean isZerocoinSpend(){
        byte[] program = getProgram();
        return program.length >0 &&
                (program[0] & 0xff) == OP_ZEROCOINSPEND;
    }
